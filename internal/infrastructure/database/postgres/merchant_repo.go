...
func (r *merchantRepo) List(ctx context.Context, f repository.ListMerchantFilter) ([]*entity.Merchant, int64, error) {
	...
	query := fmt.Sprintf(
		"SELECT id, name, email, phone, status, api_key, webhook_url, webhook_secret, webhook_signing_key, fee_config, is_production, daily_cashout_limit, deleted_at, created_at, updated_at FROM merchants%s ORDER BY created_at DESC LIMIT $%d OFFSET $%d",
		where, idx, idx+1,
	)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&m.IsProduction, &m.DailyCashoutLimit, &m.DeletedAt, &m.CreatedAt, &m.UpdatedAt,
		); err != nil {
